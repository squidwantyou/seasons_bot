...
def to_alphabet(x,y):
    return ( "ABCDE"[x],str(y+1) )
def to_number(x,y):
    if x in "ABCDE":
        return ( "ABCDE".index(x)  ,int(y)-1 )
    elif x in "abced":
        return ( "abcde".index(x)  ,int(y)-1 )
...
class Board:
    ...
    def to_png(self,filepath = '' ):
        ...
        ax.text(9, 2, logtext,horizontalalignment='center', verticalalignment='center', fontsize=24 ,fontproperties = myfont)
...
def is_legal( commands ): #list
    if len(commands)<3:
        return False
    if commands[0] in ("A","B","C","D","E","a","b","c","d","e"):
        if commands[1] in ("1","2","3","4","5"):
